...
function usage()
{
    ...
    echo "  -s|--server IP: HTTP server IP address to fetch update file from"
    echo "  -r|--remote-file FILE: .swu filename to fetch from HTTP server"
...
function dual_mode()
{
    if [[ -n "${F_REMOTE_FILENAME}" ]]; then
        if [[ -z "${F_SERVER}" ]]; then
            echo "Missing server IP for HTTP download"
            return 1
        fi
        ...
        cd /tmp
        echo "Downloading ${F_REMOTE_FILENAME} from ${F_SERVER} via HTTP..."
        wget "http://${F_SERVER}/${F_REMOTE_FILENAME}" -O "/tmp/${F_REMOTE_FILENAME}"
